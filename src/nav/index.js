...
class Nav extends Component {
  ...
  render() {
    ...
    const DirContent = total.map((item, index) => {
      ...
      return (
        <div key={index} className="nav-in-con" onClick={e => {
          e.preventDefault();
          item.hasOwnProperty('items') ?
            this.props.onClick(item.items) : console.log("none");
        }}>
          <Link
            to={item.hasOwnProperty('link') ? `${item.link}` : ''}>
            <p className="nav-title" style={num % 2 === 0 ? {"float": "left"} : {"float": "right"}}>{item.title}</p>
